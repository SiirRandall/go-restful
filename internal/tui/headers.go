...
var headerValuesMap = map[string][]string{
	...
	"Accept": {
		...
		"image/webp",
		"image/svg+xml",
		"image/x-icon", // Favicon and other icons
	...
	"Authorization": {
		"Bearer",        // Commonly used for OAuth 2.0 token-based authentication.
		"Basic",         // Base64-encoded credentials. Seen in many older HTTP services.
		"Digest",        // Challenge-based mechanism, more secure than Basic.
		"Negotiate",     // Used for Kerberos, and Microsoft's variant called NTLM.
		"OAuth",         // OAuth authentication (less common since OAuth 2.0).
		"HOBA",          // HTTP Origin-Bound Authentication, digitally signs HTTP requests.
		"Mutual",        // Mutual authentication.
		"SCRAM-SHA-1",   // Salted Challenge Response Authentication Mechanism, uses SHA-1.
		"SCRAM-SHA-256", // SCRAM using SHA-256.
		"AWC",           // Amazon Web Services request signing.
		"vapid",         // Used for WebPush protocol.
	},
	"Cache-Control": {
		"max-age=3600",     // Cache content for 1 hour
		"no-cache",         // Revalidate content with the server before using
		"no-store",         // Do not store a cached copy, always fetch from server
		"public",           // Cacheable by both browser and intermediary proxies
		"private",          // Cacheable only by the browser, not intermediary proxies
		"no-transform",     // Do not modify the content of the resource
		"must-revalidate",  // Revalidate with the server before using even if cached
		"proxy-revalidate", // Revalidate with the proxy before using even if cached
		"immutable",        // Content will not change over time
		"s-maxage=1800",    // Shared max-age for intermediary proxies (30 minutes)
		"max-stale=604800", // Accept stale content up to 1 week old (not recommended)
		"min-fresh=1800",   // Require content to be fresh for at least 30 minutes
	},
	"Connection": {
		"keep-alive",          // Keep the connection alive for future requests
		"close",               // Close the connection after the current request
		"upgrade",             // Switch to a different protocol (e.g., HTTP/2)
		"keep-alive, upgrade", // Keep alive and request an upgrade
	},
	"Content-Encoding": {
		"gzip",     // Compress using gzip
		"compress", // Compress using LZW
		"deflate",  // Compress using deflate
		"br",       // Compress using Brotli
		"identity", // No compression
		"zstd",     // Compress using Zstandard
	},
	"Content-Language": {
		"en",    // English
	...
	"Cookie": {
		"name=value",               // A cookie with a name and headerValuesMap
		"name=value; name2=value2", // Multiple cookies
	},
	"Host": {
		"example.com",      // A domain name
		"example.com:8080", // A domain name with a Portuguese
	},
	"User-Agent": {
		"--Current--",
		...
		"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Edge/90.0.818.62 Safari/537.36",                         // Microsoft Edge on Windows 10
		"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Firefox/90.0.2 Safari/537.36",                           // Firefox on Windows 10
		"Mozilla/5.0 (Macintosh; Intel Mac OS X 11_4_0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/90.0.4430.212 Safari/537.36",                // Chrome on macOS Big Sur
		"Mozilla/5.0 (Macintosh; Intel Mac OS X 11_4_0) AppleWebKit/537.36 (KHTML, like Gecko) Safari/537.36",                                     // Safari on macOS Big Sur
		"Mozilla/5.0 (iPhone; CPU iPhone OS 14_5 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/14.1 Mobile/15E148 Safari/604.1", // Safari on iPhone with iOS 14.5
		"Mozilla/5.0 (Android 11; Mobile; rv:88.0) Gecko/88.0 Firefox/88.0",                                                                       // Firefox on Android 11
		"--Others--",
	...
	"Content-Length": {
		"0",      // Empty body
		"1024",   // Content is 1024 bytes long
		"2048",   // Content is 2048 bytes long
		"4096",   // Content is 4096 bytes long
		"8192",   // Content is 8192 bytes long
		"16384",  // Content is 16384 bytes long
		"32768",  // Content is 32768 bytes long
		"65536",  // Content is 65536 bytes long
		"131072", // Content is 131072 bytes long
	},
}
